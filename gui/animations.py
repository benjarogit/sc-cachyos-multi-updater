...
class AnimationHelper:
    ...
    def create_fade_animation(widget: QWidget, start_opacity: float = 0.0, end_opacity: float = 1.0, 
                             duration: int = DURATION_FAST, easing: QEasingCurve.Type = QEasingCurve.Type.InOutCubic) -> QPropertyAnimation:
    ...
    @staticmethod
    def create_scale_animation(widget: QWidget, start_scale: float = 1.0, end_scale: float = 1.1,
                              duration: int = DURATION_FAST, easing: QEasingCurve.Type = QEasingCurve.Type.OutCubic) -> QPropertyAnimation:
    ...
    @staticmethod
    def create_color_animation(widget: QWidget, start_color: str, end_color: str,
                               duration: int = DURATION_FAST, easing: QEasingCurve.Type = QEasingCurve.Type.InOutCubic) -> QPropertyAnimation:
    ...
    @staticmethod
    def create_combined_animation(widget: QWidget, animations: list, 
...
def animate_button_hover(widget: QWidget, hover: bool = True, duration: int = AnimationHelper.DURATION_FAST):
    ...
    if hover:
        ...
        widget.setStyleSheet(widget.styleSheet().replace(" transform: scale(1.05);", ""))


def animate_dialog_show(dialog: QWidget, duration: int = AnimationHelper.DURATION_MEDIUM):
...
def animate_dialog_hide(dialog: QWidget, callback: Optional[Callable] = None, 
