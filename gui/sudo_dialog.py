...
class SudoDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Info label
        info_label = QLabel(t("gui_sudo_password_info", "Sudo password is required to perform system updates."))
        ...
        if self.save_to_config:
            if HAS_FA_CHECKBOX:
                self.save_checkbox = FACheckBox(t("gui_save_password", "Save password in settings (encrypted)"))
            else:
                self.save_checkbox = QCheckBox(t("gui_save_password", "Save password in settings (encrypted)"))
    ...
    def accept_dialog(self):
        ...
        if not password:
            QMessageBox.warning(self, t("gui_error", "Error"), t("gui_password_empty", "Password cannot be empty!"))
            return
