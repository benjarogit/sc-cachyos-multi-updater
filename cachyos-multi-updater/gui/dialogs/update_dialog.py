...
from ..utils import VersionChecker, get_logger
...
class UpdateDialog(QDialog):
    ...
    def __init__(
        ...
        self.logger.debug(
            f"UpdateDialog initialized: local={local_version}, github={github_version}"
        )
    ...
    def init_ui(self):
        ...
        local_label = QLabel(
            f"{t('gui_local_version', 'Local Version')}: v{self.local_version}"
        )
        github_label = QLabel(
            f"{t('gui_github_version', 'GitHub Version')}: v{self.github_version}"
        )
        ...
        self.radio_zip = QRadioButton(
            t("gui_auto_update", "Automatic Update (ZIP Download)")
        )
        ...
        self.asset_combo.setToolTip(
            t("gui_select_asset", "Select release asset to download")
        )
        ...
        self.radio_manual = QRadioButton(
            t("gui_manual_update", "Manual Update (Select ZIP File)")
        )
    ...
    def on_zip_selected(self, checked: bool):
        ...
        if len(zip_assets) > 0:
            ...
            for asset in zip_assets:
                ...
                self.asset_combo.addItem(
                    display_text, asset.get("browser_download_url")
                )
    ...
    def perform_manual_update(self):
        ...
        if not zip_path.exists():
            QMessageBox.warning(
                self,
                t("gui_update_failed", "Update Failed"),
                t("gui_file_not_found", "File not found: {path}").format(
                    path=file_path
                ),
            )
            return
    ...
    def perform_git_pull_update(self):
        ...
        if not self.has_git:
            QMessageBox.warning(
                ...
                t("gui_update_failed", "Update Failed"),
                t(
                    "gui_git_not_available",
                    "Git repository not found. Cannot perform Git Pull update.",
                ),
            )
            return
        ...
        reply = QMessageBox.question(
            ...
            t(
                "gui_git_pull_warning",
                "This will reset your local repository to match GitHub.\n\nAny uncommitted changes will be lost.\n\nContinue?",
            ),
        ...
        try:
            ...
            try:
                ...
                QMessageBox.information(
                    ...
                    t(
                        "gui_git_pull_success",
                        "Successfully updated via Git Pull.\n\nPlease restart the application.",
                    ),
                )
    ...
    def install_from_zip(self, zip_path: Path, temp_dir: Path):
        ...
        try:
            ...
            if target_dir.exists():
                backup_dir = (
                    self.root_dir
                    / f"cachyos-multi-updater.backup-{int(__import__('time').time())}"
                )
